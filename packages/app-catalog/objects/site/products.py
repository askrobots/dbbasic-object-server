"""Products page — built from metadata, not markup.
...
const list = window.dbbasicList("products", {
  mount: "#list", search: "#search", sort: "#sort", owner: OWNER_ID,
  title: (r) => r.name || "(no name)",
  href: (r) => "/products/" + r.id,
  subtitle: (r) => [r.sku, r.product_type, money(r.price_cents, r.currency),
    r.is_active === "false" ? "inactive" : null].filter(Boolean).join(" \\u00b7 "),
  created: (r) => r.created_at, onEdit: (r) => openForm(r),
});
function openForm(record) {
  document.getElementById("formtitle").textContent = record ? "Edit Product" : "New Product";
  panel.style.display = "block";
  window.dbbasicForm("products", {
...
def GET(request):
    ...
    _logger.info("site_products served", user_id=user_id or "anonymous")

    if not user_id:
        body = '<p class="hint"><a href="/login?next=/products">Sign in</a> to see your products.</p>'
        script = ""
    else:
        body = """
<div class="breadcrumb"><a href="/">Home</a> / Products</div>
<div class="pagehead"><h1>Products</h1><button class="btn primary" id="add">+ New Product</button></div>
<div id="formpanel" style="display:none; margin-bottom:1rem">
  <h2 id="formtitle" style="font-size:1rem; margin:0 0 0.5rem">New Product</h2>
  <div id="formmount"></div>
</div>
<div class="toolbar">
  <input class="search grow" id="search" placeholder="Search products&hellip;" autocomplete="off">
...
"""
    ...
    who = (
        ...
        if user_id
        else '<a href="/login?next=/products">sign in</a>'
    )
    html = f"""<!doctype html>
<html lang="en">
<head>
