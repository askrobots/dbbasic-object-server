"""Invoices page — built from metadata, not markup.
...
const list = window.dbbasicList("invoices", {
...
function openForm(record) {
  document.getElementById("formtitle").textContent = record ? "Edit Invoice" : "New Invoice";
  panel.style.display = "block";
  window.dbbasicForm("invoices", {
...
def GET(request):
    ...
    _logger.info("site_invoices served", user_id=user_id or "anonymous")

    if not user_id:
        body = '<p class="hint"><a href="/login?next=/invoices">Sign in</a> to see your invoices.</p>'
        script = ""
    else:
        body = """
<div class="breadcrumb"><a href="/">Home</a> / Invoices</div>
<div class="pagehead"><h1>Invoices</h1><button class="btn primary" id="add">+ New Invoice</button></div>
<div id="formpanel" style="display:none; margin-bottom:1rem">
  <h2 id="formtitle" style="font-size:1rem; margin:0 0 0.5rem">New Invoice</h2>
  <div id="formmount"></div>
</div>
<div class="toolbar">
  <input class="search grow" id="search" placeholder="Search invoices&hellip;" autocomplete="off">
...
"""
    ...
    who = (
        ...
        if user_id
        else '<a href="/login?next=/invoices">sign in</a>'
    )
    html = f"""<!doctype html>
<html lang="en">
<head>
